...
try {
    ...
    if ($filename) {
        ...
        $prod->bindValue(":prod_id", $_POST["prod_id"]);
        $prod->bindValue(":prod_name", $_POST["prod_name"]);
    
        if (isset($_POST["prod_discount_price"]) && $_POST["prod_discount_price"] !== "") {
            $prod->bindValue(":prod_discount_price", $_POST["prod_discount_price"], PDO::PARAM_INT);
        } else {
            $prod->bindValue(":prod_discount_price", null, PDO::PARAM_NULL);
        }
    ...
    $sql = "DELETE FROM prod_tag WHERE prod_id = :prod_id";
    
    $del = $pdo->prepare($sql);
    $del->bindValue(":prod_id", $_POST["prod_id"]);
    $del->execute();

    $sql = "INSERT INTO prod_tag (prod_id, tag_id)VALUES(:prod_id,:tag_id)";
    $prod_tag = $pdo->prepare($sql);
    $prod_tag->bindValue(":prod_id", $_POST["prod_id"]);
    foreach($tags as $key => $item){
        $prod_tag->bindValue(":tag_id",(int)$item);
        $prod_tag->execute();
    }

    $result = ["error" => false, "msg"=>"成功更改商品","test"=>$_POST["ppl"]];
