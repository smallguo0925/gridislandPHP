...
try {
    ...
    $table_type = $pdo->prepare($sql); //

    $table_type->bindValue(":table_amount", $table_amount); //修改桌數，將 $table_amount 變數的值綁定到 SQL 語句中的 :table_amount 參數。
    $table_type->bindValue(":table_type_id", $table_type_id); // 绑定 table_type_id

    $table_type->execute();//執行 SQL 語句，實際更新資料庫中的記錄。
