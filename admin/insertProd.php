...
try {
    ...
    if ($_FILES["news_image"]["error"] === 0) {
        ...
        $fileExt = pathinfo($_FILES["prod_image"]["name"], PATHINFO_EXTENSION);//假設是sara.png，用pathingo取出附檔名。
    ...
    $sql ="insert into prod (prod_id, prod_name, prod_price, prod_discount_price, prod_date, prod_intro, prod_brief, prod_imag1, prod_state) values (null, :prod_name, :prod_price, :prod_discount_price, :prod_date, :prod_intro, :prod_brief, :prod_imag1, :prod_state)";

    $news = $pdo->prepare( $sql );//使用prepare，避免隱碼攻擊

    $news_state = isset($_POST["news_state"]) ? $_POST["news_state"] : null; //將狀態設預設為0

    $news->bindValue(":prod_name", $_POST["prod_name"]);
    $news->bindValue(":prod_price", $_POST["prod_price"]);
    $news->bindValue(":prod_discount_price", $_POST["prod_discount_price"]);
    $news->bindValue(":prod_date", $_POST["prod_date"]);
    $news->bindValue(":prod_intro", $_POST["prod_intro"]);
    $news->bindValue(":prod_brief", $_POST["prod_brief"]);
    $news->bindValue(":prod_imag1", $filename);
    $news->bindValue(":prod_state", $prod_state);
